...
- `x`: A data frame containing pupil data with columns `time_secs` and
  the previous operation’s pupil column
- `prev_op`: The name of the previous operation’s pupil column
- `n`: The constant used to compute the median absolute deviation (MAD)
  threshold (default: 16)
- `mad_thresh`: Optional threshold override

**Returns**: A numeric vector with transient artifacts removed (set to
NA)
...
eyeris:::interpolate_pupil(x, prev_op, verbose, max_gap_ms = 250)
```
...
- `max_gap_ms`: Maximum gap duration (ms) to interpolate; longer gaps
  are left as `NA` (use `Inf`/`NULL` to interpolate across all gaps)
...
**Returns**: A list containing the downsampled data and the decimated
sample rate
...
**Returns**: A list containing the fitted values, coefficients, and
residuals
...
- `label`: Epoch label
- `c_bline`, `a_bline`: **Deprecated** baseline calculation and
  application options
...
**Returns**: A list containing epoch and baseline results for the
specified block
...
**Returns**: A data frame containing confounds metrics for the current
step
...
**Returns**: Character string with current timestamp in format
`[YYYY-MM-DD HH:MM:SS]`
...
Many internal functions are designed to work within the pipeline
architecture:
...
eyeris_obj |>
  ...
  eyeris:::interpolate_pupil(prev_op = "pupil_raw_deblink_detransient", verbose = TRUE, max_gap_ms = 250)
```
...
2.  **Documentation**: Always refer to this documentation for current
    signatures
3.  **Testing**: Test thoroughly when using internal functions directly
4.  **Error Handling**: Implement appropriate error handling in your
    code
5.  **Performance**: Consider performance implications of bypassing the
    main pipeline
