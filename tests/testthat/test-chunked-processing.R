...
test_that("column structure grouping works correctly", {
  ...
  output_files <- list.files(output_dir, pattern = ".*_confounds_summary_.*_chunked.*\\.csv$", full.names = TRUE)
  
  expect_true(length(output_files) >= 2, info = paste("Expected at least 2 files, got:", length(output_files)))
  ...
  if (length(output_files) >= 2) {
    ...
    cols1 <- setdiff(colnames(data1), c("subject_id", "session_id", "task_name", "data_type"))
    cols2 <- setdiff(colnames(data2), c("subject_id", "session_id", "task_name", "data_type"))
    
    expect_false(identical(sort(cols1), sort(cols2)), 
...
test_that("file size limits create numbered files", {
  ...
  if ("files" %in% names(timeseries_info)) {
    ...
    for (file_path in timeseries_info$files) {
      ...
      expect_true(file_size_mb <= 0.1, # Allow some overhead
                  info = paste("File too large:", basename(file_path), "-", round(file_size_mb, 3), "MB"))
    }
