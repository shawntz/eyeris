...
print_lightbox_img_html <- function(zip_path, image_filenames = NULL) {
  ...
  html_out <- paste0(
    ...
    "        }\n",
    "      } else {\n",
    "        galleryDiv.innerHTML = '<div style=\"padding: 15px; border: 2px solid #f39c12; background-color: #fdf6e3; margin: 10px 0; border-radius: 5px;\"><p style=\"color: #d68910; font-weight: bold;\">⚠️ Please select a valid ZIP file</p></div>';\n",
    "      }\n",
    "    });\n",
    "  }\n",
    "})();\n",
