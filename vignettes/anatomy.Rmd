...
- `timeseries`: a list of data frames (1 df per identified recording block per file) which contains the following columns:
  ...
  - `is_resampled`: logical flag marking rows that `resample()` inserted for dropped samples (present *only* when the sampling grid had to be repaired -- see the `glassbox()` steps below)
...
| 2. Resample | `eyeris::resample()` | Place each block on the expected uniform sampling grid (repair dropped samples; no-op if already uniform) |
| 3. Deblink | `eyeris::deblink()` | NA-pad samples surrounding blink artifacts |
| 4. Detransient | `eyeris::detransient()` | Remove physiologically implausible jumps |
| 5. Interpolate | `eyeris::interpolate()` | Linearly fill in missing samples |
| 6. Lowpass filter | `eyeris::lpfilt()` | Smooth the pupil time series |
...
| _(optional)_ Detrend | `eyeris::detrend()` | Remove a linear or spline trend from the time series |
| 7. Z-score | `eyeris::zscore()` | Standardize to mean 0 and SD 1 |
