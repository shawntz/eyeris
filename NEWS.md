...
- **ENH (#320)**: Added an automatic `regularize()` step that repairs the **time axis** of recordings whose hardware *drops* samples (instead of zero-filling) when pupil data is missing — the dropped-sample quirk surfaced by the #300 guardrail. When `glassbox()` detects irregular sampling intervals (gated on the robust `check_uniform_sampling_intervals()` detector from #300), it rebuilds each block's uniform grid and inserts placeholder rows at the dropped timestamps, flagged in a new logical `is_resampled` column and left as `NA` for the existing `interpolate()` step to fill. This turns the "dropped-sample" problem into the ordinary "missing-value" problem the rest of the pipeline already handles. It runs **automatically and by default**, but only when needed: it is a guaranteed no-op for already-uniform data (e.g., EyeLink) and deliberately leaves high-rate recordings that report integer-millisecond timestamps untouched (so it never collapses genuine sub-millisecond samples). Opt out with `glassbox(regularize = FALSE)`; a `max_inflation` guard refuses to fabricate data for systematically-decimated recordings. `confounds()` now also reports `n_resampled`/`prop_resampled` per block, by @shawntz in #320.
