...
library(eyeris)
#> 
#> eyeris v3.3.0 - Lumpy Space Princess ꒰•ᴗ•｡꒱۶
...
#> ✔ [2026-08-13 18:07:12] [OKAY] Running eyeris::load_asc()
#> ✔ [2026-08-13 18:07:13] [OKAY] Running eyeris::resample()
#> ℹ [2026-08-13 18:07:13] [INFO] Processing block: block_1
#> ✔ [2026-08-13 18:07:13] [OKAY] Running eyeris::deblink() for block_1
#> ✔ [2026-08-13 18:07:13] [OKAY] Running eyeris::detransient() for block_1
#> ✔ [2026-08-13 18:07:13] [OKAY] Running eyeris::interpolate() for block_1
#> ! [2026-08-13 18:07:13] [WARN] Interpolation now leaves gaps longer than 250 ms
...
#> `interpolate()`).
#> ✔ [2026-08-13 18:07:13] [OKAY] Running eyeris::lpfilt() for block_1
```

![](complete-pipeline_files/figure-html/unnamed-chunk-3-1.png)

    #> ! [2026-08-13 18:07:13] [WARN] Skipping eyeris::downsample() for block_1
    #> ! [2026-08-13 18:07:13] [WARN] Skipping eyeris::bin() for block_1
    #> ! [2026-08-13 18:07:13] [WARN] Skipping eyeris::detrend() for block_1
    #> ✔ [2026-08-13 18:07:13] [OKAY] Running eyeris::zscore() for block_1
    #> ℹ [2026-08-13 18:07:13] [INFO] Block processing summary:
    #> ℹ [2026-08-13 18:07:13] [INFO] block_1: OK (steps: 6, latest:
    #> pupil_raw_deblink_detransient_interpolate_lpfilt_z)
    #> ✔ [2026-08-13 18:07:13] [OKAY] Running eyeris::summarize_confounds()
    ...
    #> ℹ [2026-08-13 18:07:13] [INFO] Plotting block 1 with sampling rate 1000 Hz from
    #> possible blocks: 1
...
Before any rate-dependent step runs,
[`glassbox()`](https://eyeris.shawnschwartz.com/reference/glassbox.md)
places each recording block onto the expected **uniform sampling grid**
via
[`eyeris::resample()`](https://eyeris.shawnschwartz.com/reference/resample.md).
Most trackers (e.g., EyeLink) zero-fill missing pupil samples, so their
time vector is already evenly spaced and this step is a guaranteed
**no-op**. Some hardware instead *drops* samples when pupil data is
missing, leaving holes that would silently distort filtering,
downsampling, and any other step that assumes a fixed sampling rate.

When irregular sampling is detected,
[`resample()`](https://eyeris.shawnschwartz.com/reference/resample.md)
repairs the time axis in two stages: it anchors a uniform grid on the
first *reliable* regular interval (so early timing jitter doesn’t offset
the whole grid), interpolates local sub-period jitter onto that grid,
and inserts `NA` rows at any longer-than-expected gaps (the dropped
samples). Those `NA` gaps are then filled by the
[`interpolate()`](https://eyeris.shawnschwartz.com/reference/interpolate.md)
step later in the pipeline, and the inserted rows are flagged in an
`is_resampled` column (also surfaced by
[`summarize_confounds()`](https://eyeris.shawnschwartz.com/reference/summarize_confounds.md)
as `n_resampled` / `prop_resampled`).
...
#> ✔ [2026-08-13 18:07:18] [OKAY] Running eyeris::load_asc()
#> ✔ [2026-08-13 18:07:18] [OKAY] Running eyeris::resample()
#> ℹ [2026-08-13 18:07:18] [INFO] Processing block: block_1
#> ✔ [2026-08-13 18:07:18] [OKAY] Running eyeris::deblink() for block_1
#> ✔ [2026-08-13 18:07:18] [OKAY] Running eyeris::detransient() for block_1
#> ✔ [2026-08-13 18:07:18] [OKAY] Running eyeris::interpolate() for block_1
#> ! [2026-08-13 18:07:18] [WARN] Interpolation now leaves gaps longer than 250 ms
#> as `NA` instead of interpolating across them (following Kret & Sjak-Shie,
#> 2018). This is a change in default behavior from eyeris <= 3.2.0 and may affect
#> your results. To restore the previous behavior, set `interpolate =
#> list(max_gap_ms = Inf)` in `glassbox()` (or `max_gap_ms = Inf` in
#> `interpolate()`).
#> ✔ [2026-08-13 18:07:18] [OKAY] Running eyeris::lpfilt() for block_1
#> ! [2026-08-13 18:07:18] [WARN] Skipping eyeris::downsample() for block_1
#> ! [2026-08-13 18:07:18] [WARN] Skipping eyeris::bin() for block_1
#> ! [2026-08-13 18:07:18] [WARN] Skipping eyeris::detrend() for block_1
#> ✔ [2026-08-13 18:07:18] [OKAY] Running eyeris::zscore() for block_1
#> ℹ [2026-08-13 18:07:18] [INFO] Block processing summary:
#> ℹ [2026-08-13 18:07:18] [INFO] block_1: OK (steps: 6, latest:
#> pupil_raw_deblink_detransient_interpolate_lpfilt_z)
#> ✔ [2026-08-13 18:07:18] [OKAY] Running eyeris::summarize_confounds()
```
...
> [`glassbox()`](https://eyeris.shawnschwartz.com/reference/glassbox.md).
> This removes a straight-line (linear) trend by default. To instead
> remove a smooth, potentially nonlinear trend with a natural cubic
> spline of time, pass `detrend = list(method = "spline")` (optionally
> tuning the spline degrees of freedom with `spline_df`,
