...
The `eyeris_extract_data()` function provides easy one-liner access to your data:

```{r simple-extraction}
# Extract ALL data for ALL subjects (returns a named list)
all_data <- eyeris_extract_data("~/processed_study_data", "attention_study_db")
...
subset_subjects <- eyeris_extract_data(
  bids_dir = "~/processed_study_data",
  db_path = "attention_study_db",
...
behavioral_data <- eyeris_extract_data(
  bids_dir = "~/processed_study_data", 
  db_path = "attention_study_db",
...
session_01_data <- eyeris_extract_data(
  bids_dir = "~/processed_study_data",
  db_path = "attention_study_db", 
  sessions = "01",
...
binocular_data <- eyeris_extract_data(
  bids_dir = "~/processed_study_data",
  db_path = "attention_study_db"
)
...
left_eye_data <- eyeris_extract_data(
  bids_dir = "~/processed_study_data",
  db_path = "attention_study_db", 
  eye_suffixes = "eyeL"
)
...
trial_epochs <- eyeris_extract_data(
  bids_dir = "~/processed_study_data",
  db_path = "attention_study_db",
...
multiple_epochs <- eyeris_extract_data(
  bids_dir = "~/processed_study_data", 
  db_path = "attention_study_db",
  data_types = "epochs",
...
list_format <- eyeris_extract_data(
...
combined_format <- eyeris_extract_data(
  "~/processed_study_data", 
  return_list = FALSE
)
...
pupil_data <- eyeris_extract_data(
  "~/processed_study_data",
...
confounds_data <- eyeris_extract_data(
  "~/processed_study_data",
...
process_data <- function() {
  ...
  results <- eyeris_extract_data("~/data", "study_db")
  return(results)
}
```
...
analyze_cloud_data <- function() {
  ...
  analysis_data <- eyeris_extract_data(
    "local_processing",
...
safe_extract <- function(bids_dir, db_path, ...) {
  tryCatch({
    data <- eyeris_extract_data(bids_dir, db_path, ...)
    return(data)
  }, error = function(e) {
...
Whether you're running a small pilot study or a large-scale multi-site experiment, eyeris databases can streamline your workflow and improve analysis efficiency. Start with the simple `eyeris_extract_data()` function for most use cases, and leverage direct SQL queries when you need maximum flexibility.
