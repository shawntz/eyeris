...
eyeris_db_to_parquet <- function(
  ...
  for (data_type in names(data_type_groups)) {
    ...
    # collect data for this data type
    type_data_list <- list()
    type_total_rows <- 0
      ...
      tryCatch(
        {
          ...
          if (nrow(data) > 0) {
            ...
            # estimate size
            estimated_size_mb <- object.size(data) / (1024^2)
            type_total_size_mb <- type_total_size_mb +
              as.numeric(estimated_size_mb)
          }
        },
        error = function(e) {
          log_warn(
    ...
    # combine all tables for this data type (align by column names; fill missing)
